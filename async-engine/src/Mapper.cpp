...
Poco::Dynamic::Var Mapper::map(emscripten::val value) {
    ...
    if (type == "number") {
        if (checkIfJsValueIsSafeInteger(value.as_handle())) {
            emscripten::val canonicalInteger = emscripten::val::take_ownership(
                convertJsSafeIntegerToCanonicalInteger(value.as_handle()));
    ...
    if (type == "object") {
        emscripten::val keys =
            emscripten::val::take_ownership(listKeysOfJsObject(value.as_handle()));
