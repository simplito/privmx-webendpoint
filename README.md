...
const threads = await Endpoint.createThreadApi(conn);
...
You drive everything through **`Endpoint`** (the `EndpointFactory` static facade):
`setup` / `setupAuto`, `connect`, `createThreadApi`, `createStoreApi`, and so on.
...
and an audio worklet). With the **`/auto` entry** you don't copy them anywhere —
`setupAuto()` resolves them via `import.meta.url` and Vite fingerprints and serves
them automatically:
...
```ts
import { setupAuto } from "@simplito/privmx-webendpoint/auto";
...
for (;;) {
  const event = await queue.waitEvent();   // resolves when an event arrives
  if (event.type === "libBreak") break;    // emitted by queue.emitBreakEvent()
  console.log(event.channel, event.type, event.data);
}
```

For a higher-level option, `/extra` provides `EventManager.startEventLoop(queue)`
and per-domain managers (`getThreadEventManager(threadApi).subscribeFor([…])`) that
run the loop and dispatch to callbacks for you — see the
[example](example/vite) and the API reference.
...
```
setup()  -->  connect() / connectPublic()  -->  createXApi(connection)  -->  …work…  -->  connection.disconnect()
...
bundler, and `import.meta.url`-based asset loading via the `/auto` entry. Use it
...
- **Getting Started:** [Introduction to PrivMX JS](https://docs.privmx.dev/docs/latest/js/introduction)
