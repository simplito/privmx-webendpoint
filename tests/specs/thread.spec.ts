...
test.describe('ThreadTest', () => {
    ...
    test('Listing threads with invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // incorrect contextId
            await expectError(async () => await threadApi.listThreads("invalid_value", {skip: 0, limit: 1, sortOrder: "desc"}));
            // limit < 0
            await expectError(async () => await threadApi.listThreads(contextId, {skip: 0, limit: -1, sortOrder: "desc"}));
            // limit == 0
            await expectError(async () => await threadApi.listThreads(contextId, {skip: 0, limit: 0, sortOrder: "desc"}));
            // incorrect sortOrder
            // @ts-ignore
            await expectError(async () => await threadApi.listThreads(contextId, {skip: 0, limit: 1, sortOrder: "invalid_value"}));
            // incorrect lastId
            // @ts-ignore
            await expectError(async () => await threadApi.listThreads(contextId, {skip: 0, limit: 1, sortOrder: "desc", lastId: "invalid_value"}));
        }, args);
    });
    ...
    test('Listing threads with valid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const tids: string[] = [];
            for(let i=0; i<3; i++) {
                tids.push(await threadApi.createThread(contextId, [u1Obj], [u1Obj], enc.encode(`p${i}`), enc.encode(`p${i}`)));
            }

            // Test Pagination
            const list1 = await threadApi.listThreads(contextId, {skip: 4, limit: 1, sortOrder: "desc"});
            const list2 = await threadApi.listThreads(contextId, {skip: 0, limit: 1, sortOrder: "desc"}); // Newest
            const list3 = await threadApi.listThreads(contextId, {skip: 1, limit: 3, sortOrder: "asc"}); // 2nd oldest onwards
    ...
    test('Creating thread with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // incorrect contextId
            await expectError(async () => await threadApi.createThread("invalid", [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p")));
            // incorrect users (key mismatch)
            await expectError(async () => await threadApi.createThread(contextId, [{userId: u1Obj.userId, pubKey: u2Obj.pubKey}], [u1Obj], enc.encode("p"), enc.encode("p")));
            // incorrect managers (key mismatch)
            await expectError(async () => await threadApi.createThread(contextId, [u1Obj], [{userId: u1Obj.userId, pubKey: u2Obj.pubKey}], enc.encode("p"), enc.encode("p")));
            // no managers
            await expectError(async () => await threadApi.createThread(contextId, [u1Obj], [], enc.encode("p"), enc.encode("p")));

            // different users and managers
            const t1 = await threadApi.createThread(contextId, [u2Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            const thread1 = await threadApi.getThread(t1);

            // same users and managers
            const t2 = await threadApi.createThread(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
    ...
    test('Updating thread with invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // incorrect threadId
            await expectError(async () => await threadApi.updateThread("invalid", [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), tData.version, false, false));
            // incorrect users
            await expectError(async () => await threadApi.updateThread(tId, [{userId: u1Obj.userId, pubKey: u2Obj.pubKey}], [u1Obj], enc.encode("p"), enc.encode("p"), tData.version, false, false));
            // incorrect managers
            await expectError(async () => await threadApi.updateThread(tId, [u1Obj], [{userId: u1Obj.userId, pubKey: u2Obj.pubKey}], enc.encode("p"), enc.encode("p"), tData.version, false, false));
            // no managers
            await expectError(async () => await threadApi.updateThread(tId, [u1Obj], [], enc.encode("p"), enc.encode("p"), tData.version, false, false));
            // incorrect version (force false)
            await expectError(async () => await threadApi.updateThread(tId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), tData.version + 1, false, false));
    ...
    test('Updating thread with new users', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Add User 2
            await threadApi.updateThread(tId, [u1Obj, u2Obj], [u1Obj], enc.encode("p"), enc.encode("p"), tV1.version, false, false);
    ...
    test('Updating thread with new managers', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Add Manager 2
            await threadApi.updateThread(tId, [u1Obj], [u1Obj, u2Obj], enc.encode("p"), enc.encode("p"), tV1.version, false, false);
    ...
    test('Updating thread with less users', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Create with 2 users
            const tId = await threadApi.createThread(contextId, [u1Obj, u2Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            const tV1 = await threadApi.getThread(tId);

            // Remove User 2
            await threadApi.updateThread(tId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), tV1.version, false, false);
    ...
    test('Updating thread with less managers', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const tId = await threadApi.createThread(contextId, [u1Obj], [u1Obj, u2Obj], enc.encode("p"), enc.encode("p"));
            const tV1 = await threadApi.getThread(tId);
    ...
    test('Force updating thread with incorrect version', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await threadApi.updateThread(tId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), tV1.version + 1, true, false);
    ...
    test('Deleting thread with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Setup
            const t1 = await api1.createThread(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            const t2 = await api1.createThread(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
    ...
    test('Getting message with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const tId = await threadApi.createThread(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            const msgId = await threadApi.sendMessage(tId, enc.encode("p"), enc.encode("p"), enc.encode("d"));
    ...
    test('Listing messages with invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // incorrect threadId
            await expectError(async () => await threadApi.listMessages("invalid", {skip:0, limit:1, sortOrder:"desc"}));
            // limit < 0
            await expectError(async () => await threadApi.listMessages(contextId, {skip:0, limit:-1, sortOrder:"desc"})); // Using ctx ID as thread ID placeholder for invalid check
            // incorrect sortOrder
            // @ts-ignore
            await expectError(async () => await threadApi.listMessages(contextId, {skip:0, limit:1, sortOrder:"invalid"}));
            // incorrect lastId
            // @ts-ignore
            await expectError(async () => await threadApi.listMessages(contextId, {skip:0, limit:1, sortOrder:"desc", lastId:"invalid"}));
    ...
    test('Listing message with valid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Send 2 messages
            const m1 = await threadApi.sendMessage(tId, enc.encode("p"), enc.encode("p"), enc.encode("1"));
            const m2 = await threadApi.sendMessage(tId, enc.encode("p"), enc.encode("p"), enc.encode("2"));

            const list1 = await threadApi.listMessages(tId, {skip: 2, limit: 1, sortOrder: "desc"}); // Empty
            const list2 = await threadApi.listMessages(tId, {skip: 0, limit: 1, sortOrder: "desc"}); // Last (m2)
            const list3 = await threadApi.listMessages(tId, {skip: 0, limit: 2, sortOrder: "asc"}); // Both
    ...
    test('Sending message with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Incorrect Thread ID
            await expectError(async () => await threadApi.sendMessage("invalid", enc.encode("p"), enc.encode("p"), enc.encode("d")));

            // Msg too big (>1MB)
            const bigData = new Uint8Array(1024*1024 + 10); 
            await expectError(async () => await threadApi.sendMessage(tId, enc.encode("p"), enc.encode("p"), bigData));
    ...
    test('Updating message with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Incorrect Msg ID
            await expectError(async () => await threadApi.updateMessage("invalid", enc.encode("p"), enc.encode("p"), enc.encode("d")));
            
            // Msg too big
            const bigData = new Uint8Array(1024*1024 + 10);
            await expectError(async () => await threadApi.updateMessage(msgId, enc.encode("p"), enc.encode("p"), bigData));

            // Correct
            await threadApi.updateMessage(msgId, enc.encode("p"), enc.encode("p"), enc.encode("updated"));
    ...
    test('Deleting message with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const tId = await api1.createThread(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            const msgId = await api1.sendMessage(tId, enc.encode("p"), enc.encode("p"), enc.encode("d"));
    ...
    test('Trying to accessing unavailable data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const tId = await api1.createThread(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            const mId = await api1.sendMessage(tId, enc.encode("p"), enc.encode("p"), enc.encode("d"));

            const expectError = async (fn: any) => { try { await fn(); } catch { return; } throw new Error("Expected error"); };

            // User 2 (Stranger) tries to do everything
            await expectError(async () => await api2.getThread(tId));
            await expectError(async () => await api2.updateThread(tId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), 1, false, false));
            await expectError(async () => await api2.deleteThread(tId));
            await expectError(async () => await api2.getMessage(mId));
            await expectError(async () => await api2.listMessages(tId, {skip:0, limit:1, sortOrder:"desc"}));
            await expectError(async () => await api2.sendMessage(tId, enc.encode("p"), enc.encode("p"), enc.encode("d")));
            await expectError(async () => await api2.updateMessage(mId, enc.encode("p"), enc.encode("p"), enc.encode("d")));
    ...
    test.fixme('Trying to accessing data using publicAccount', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await expectError(async () => await apiPub.listThreads(contextId, {skip:0, limit:1, sortOrder:"desc"}));
            await expectError(async () => await apiPub.createThread(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p")));
    ...
    test('Creating/Updating thread with "owner" only policy', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const tId = await api1.createThread(contextId, usersArr, usersArr, enc.encode("p"), enc.encode("p"), ownerPolicy);
    ...
    test('Updating thread with "all" only policy', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const tId = await api1.createThread(contextId, usersArr, usersArr, enc.encode("p"), enc.encode("p"), policy);
            ...
            await api1.updateThread(tId, usersArr, usersArr, enc.encode("p"), enc.encode("p"), t1.version, true, true, allPolicy);

            // Now User 2 can update
            await api2.updateThread(tId, usersArr, usersArr, enc.encode("p"), enc.encode("p"), t1.version + 1, true, true, allPolicy);
        }, args);
    });
    ...
    test('Listing Threads using queryAsJson', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await api.createThread(contextId, [u1Obj], [u1Obj], enc.encode(jsonMeta), enc.encode("p"));
            
            const list = await api.listThreads(contextId, {
                skip: 0, limit: 100, sortOrder: "asc", 
    ...
    test('Checking if cache can detect old keys (sendMessage)', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const tId = await api.createThread(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            await api.getThread(tId);

            // 2. Update with Force New Keys
            const t1 = await api.getThread(tId);
            await api.updateThread(tId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), t1.version, true, true);

            // 3. Send Message (Should recover from Old Key error)
            const mId = await api.sendMessage(tId, enc.encode("p"), enc.encode("p"), enc.encode("data"));
    ...
    test('Checking if cache can detect old keys (updateMessage)', async ({ page, backend, cli }) => {
        const users = await setupUsers(page, cli);
        const args = { bridgeUrl: backend.bridgeUrl, solutionId: testData.solutionId, contextId: testData.contextId, users };
    ...
    test('Using api with UserVerifier that always return false', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Should fail actions
            const expectError = async (fn: any) => { try { await fn(); } catch { return; } throw new Error("Expected error"); };
            await expectError(async () => await api.sendMessage(tId, enc.encode("p"), enc.encode("p"), enc.encode("d")));
