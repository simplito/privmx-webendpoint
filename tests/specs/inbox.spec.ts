...
test.describe('InboxTest', () => {
    ...
    test('Getting inbox with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Setup
            const inboxId = await inboxApi.createInbox(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
    ...
    test('Listing inboxes with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const ids: string[] = [];
            for(let i=0; i<3; i++) {
                ids.push(await inboxApi.createInbox(contextId, [u1Obj], [u1Obj], enc.encode(`p${i}`), enc.encode(`p${i}`)));
            }

            const expectError = async (fn: any) => { try { await fn(); } catch { return; } throw new Error("Expected error"); };

            // --- Invalid ---
            await expectError(async () => await inboxApi.listInboxes("invalid", {skip:0, limit:1, sortOrder:"desc"}));
            await expectError(async () => await inboxApi.listInboxes(contextId, {skip:0, limit:-1, sortOrder:"desc"}));
            // @ts-ignore
            await expectError(async () => await inboxApi.listInboxes(contextId, {skip:0, limit:1, sortOrder:"invalid"}));

            // --- Valid ---
            const list1 = await inboxApi.listInboxes(contextId, {skip: 4, limit: 1, sortOrder: "desc"});
            const list2 = await inboxApi.listInboxes(contextId, {skip: 0, limit: 1, sortOrder: "desc"});
            const list3 = await inboxApi.listInboxes(contextId, {skip: 1, limit: 3, sortOrder: "asc"});
    ...
    test('Creating inbox with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Invalid Inputs
            await expectError(async () => await inboxApi.createInbox("invalid", [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p")));
            await expectError(async () => await inboxApi.createInbox(contextId, [{userId: users.u1.id, pubKey: users.u2.pubKey}], [u1Obj], enc.encode("p"), enc.encode("p"))); // Mismatch
            await expectError(async () => await inboxApi.createInbox(contextId, [u1Obj], [], enc.encode("p"), enc.encode("p"))); // No managers

            // Valid: Diff users/managers
            const i1 = await inboxApi.createInbox(contextId, [u2Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            const inbox1 = await inboxApi.getInbox(i1);

            // Valid: Same users
            const i2 = await inboxApi.createInbox(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
    ...
    test('Updating inbox (Invalid inputs & Users/Managers)', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const iId = await inboxApi.createInbox(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            const iV1 = await inboxApi.getInbox(iId);
            const expectError = async (fn: any) => { try { await fn(); } catch { return; } throw new Error("Expected error"); };

            // Invalid Inputs
            await expectError(async () => await inboxApi.updateInbox("invalid", [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), undefined, 1, false, false));
            await expectError(async () => await inboxApi.updateInbox(iId, [u1Obj], [], enc.encode("p"), enc.encode("p"), undefined, 1, false, false));
            await expectError(async () => await inboxApi.updateInbox(iId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), undefined, 99, false, false)); // Version

            // Add User 2
            await inboxApi.updateInbox(iId, [u1Obj, u2Obj], [u1Obj], enc.encode("p"), enc.encode("p"), undefined, iV1.version, false, false);
            const iV2 = await inboxApi.getInbox(iId);

            // Add Manager 2
            await inboxApi.updateInbox(iId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), undefined, iV2.version, false, false); // Removed U2 from users but kept as manager? 
            // Note: In logic, updating managers implies updating access.
            // Let's do explicit Add Manager
            await inboxApi.updateInbox(iId, [u1Obj], [u1Obj, u2Obj], enc.encode("p"), enc.encode("p"), undefined, iV2.version+1, false, false);
    ...
    test('Force updating inbox', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await inboxApi.updateInbox(iId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), undefined, 99, true, false);
    ...
    test('Deleting inbox with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const iId = await api1.createInbox(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            const expectError = async (fn: any) => { try { await fn(); } catch { return; } throw new Error("Expected error"); };
    ...
    test('Entry Flow: Send (with chunks), Read, List', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const fh2 = await inboxApi.createFileHandle(enc.encode("pub2"), enc.encode("priv2"), fileSize);

            // 2. Prepare Entry
            const inboxHandle = await inboxApi.prepareEntry(iId, enc.encode("test_entry"), [fh1, fh2], users.u1.privKey);

            // 3. Write Data
            let uploadedHex = "";
            const toHex = (u8: Uint8Array) => {
                let s = ""; for(let b of u8) s += ("0" + b.toString(16)).slice(-2).toUpperCase(); return s;
            };
            ...
            await expectError(async () => await inboxApi.listEntries("invalid", {skip:0, limit:1, sortOrder:"desc"}));
    ...
    test.fixme('Sending entry as Public user', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const h = await apiPub.prepareEntry(iId, enc.encode("pub_entry"), [fh], users.u1.pubKey); // Encrypting for target (User1)
    ...
    test('Deleting entry (Permissions)', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            const Endpoint = window.Endpoint;
            const conn1 = await Endpoint.connect(users.u1.privKey, solutionId, bridgeUrl);
            const api1 = await Endpoint.createInboxApi(conn1, await Endpoint.createThreadApi(conn1), await Endpoint.createStoreApi(conn1));
            
            const conn2 = await Endpoint.connect(users.u2.privKey, solutionId, bridgeUrl);
            const api2 = await Endpoint.createInboxApi(conn2, await Endpoint.createThreadApi(conn2), await Endpoint.createStoreApi(conn2));
    ...
    test.fixme('Access Control: Unauthorized & Public', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Public
            const connPub = await Endpoint.connectPublic(solutionId, bridgeUrl);
            const apiPub = await Endpoint.createInboxApi(connPub, await Endpoint.createThreadApi(connPub), await Endpoint.createStoreApi(connPub));

            const expectError = async (fn: any) => { try { await fn(); } catch { return; } throw new Error("Expected error"); };

            // User 2 Denied
            await expectError(async () => await api2.getInbox(iId));
            await expectError(async () => await api2.updateInbox(iId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), undefined, 2, false, false));
            await expectError(async () => await api2.deleteInbox(iId));
            await expectError(async () => await api2.listEntries(iId, {skip:0, limit:1, sortOrder:"desc"}));

            // Public Denied
            await expectError(async () => await apiPub.getInbox(iId));
            await expectError(async () => await apiPub.createInbox(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p")));
    ...
    test('Creating/Updating inbox with owner only policy', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const iId = await api1.createInbox(contextId, usersArr, usersArr, enc.encode("p"), enc.encode("p"), undefined, policy);
            ...
            let u2Access = false;
            try { await api2.getInbox(iId); u2Access = true; } catch {}

            // Update Policy check (keep strict)
            await api1.updateInbox(iId, usersArr, usersArr, enc.encode("p"), enc.encode("p"), undefined, 1, true, true, policy);
