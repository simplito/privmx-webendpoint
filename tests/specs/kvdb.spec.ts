...
test.describe('KvdbTest', () => {
    ...
    test('Getting kvdb with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Setup
            const kId = await kvdbApi.createKvdb(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
    ...
    test('Listing kvdbs with invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await expectError(async () => await kvdbApi.listKvdbs("invalid", {skip:0, limit:1, sortOrder:"desc"}));
            await expectError(async () => await kvdbApi.listKvdbs(contextId, {skip:0, limit:-1, sortOrder:"desc"}));
            await expectError(async () => await kvdbApi.listKvdbs(contextId, {skip:0, limit:0, sortOrder:"desc"}));
            // @ts-ignore
            await expectError(async () => await kvdbApi.listKvdbs(contextId, {skip:0, limit:1, sortOrder:"invalid"}));
            // @ts-ignore
            await expectError(async () => await kvdbApi.listKvdbs(contextId, {skip:0, limit:1, sortOrder:"desc", lastId:"invalid"}));
        }, args);
    });
    ...
    test('Listing kvdbs with valid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const ids = [];
            for(let i=0; i<3; i++) {
                ids.push(await kvdbApi.createKvdb(contextId, [u1Obj], [u1Obj], enc.encode(`p${i}`), enc.encode(`p${i}`)));
            }

            const list1 = await kvdbApi.listKvdbs(contextId, {skip: 4, limit: 1, sortOrder: "desc"});
            const list2 = await kvdbApi.listKvdbs(contextId, {skip: 0, limit: 1, sortOrder: "desc"});
            const list3 = await kvdbApi.listKvdbs(contextId, {skip: 1, limit: 3, sortOrder: "asc"});
    ...
    test('Creating kvdb with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await expectError(async () => await kvdbApi.createKvdb("invalid", [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p")));
            // Mismatch user key
            await expectError(async () => await kvdbApi.createKvdb(contextId, [{userId: users.u1.id, pubKey: users.u2.pubKey}], [u1Obj], enc.encode("p"), enc.encode("p")));
            // Mismatch manager key
            await expectError(async () => await kvdbApi.createKvdb(contextId, [u1Obj], [{userId: users.u1.id, pubKey: users.u2.pubKey}], enc.encode("p"), enc.encode("p")));
            // No managers
            await expectError(async () => await kvdbApi.createKvdb(contextId, [u1Obj], [], enc.encode("p"), enc.encode("p")));

            // Create Diff
            const k1 = await kvdbApi.createKvdb(contextId, [u2Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            const kvdb1 = await kvdbApi.getKvdb(k1);

            // Create Same
            const k2 = await kvdbApi.createKvdb(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
    ...
    test('Updating kvdb with invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await expectError(async () => await kvdbApi.updateKvdb("invalid", [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), 1, false, false));
            await expectError(async () => await kvdbApi.updateKvdb(kId, [{userId: u1Obj.userId, pubKey: u2Obj.pubKey}], [u1Obj], enc.encode("p"), enc.encode("p"), 1, false, false));
            await expectError(async () => await kvdbApi.updateKvdb(kId, [u1Obj], [{userId: u1Obj.userId, pubKey: u2Obj.pubKey}], enc.encode("p"), enc.encode("p"), 1, false, false));
            await expectError(async () => await kvdbApi.updateKvdb(kId, [u1Obj], [], enc.encode("p"), enc.encode("p"), 1, false, false));
            await expectError(async () => await kvdbApi.updateKvdb(kId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), 99, false, false));
    ...
    test('Updating kvdb with new users', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await kvdbApi.updateKvdb(kId, [u1Obj, u2Obj], [u1Obj], enc.encode("p"), enc.encode("p"), 1, false, false);
    ...
    test('Updating kvdb with new managers', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await kvdbApi.updateKvdb(kId, [u1Obj], [u1Obj, u2Obj], enc.encode("p"), enc.encode("p"), 1, false, false);
            return { kvdb: await kvdbApi.getKvdb(kId) };
        }, args);
    ...
    test('Updating kvdb with less user', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const kId = await kvdbApi.createKvdb(contextId, [u1Obj, u2Obj], [u1Obj, u2Obj], enc.encode("p"), enc.encode("p"));
    ...
    test('Force updating kvdb with incorrect version', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await kvdbApi.updateKvdb(kId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), 99, true, false);
            return { kvdb: await kvdbApi.getKvdb(kId) };
        }, args);
    ...
    test('Deleting kvdb with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const kId = await api1.createKvdb(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            const expectError = async (fn: any) => { try { await fn(); } catch { return; } throw new Error("Expected error"); };
    ...
    test('Getting entry with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const kId = await kvdbApi.createKvdb(contextId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"));
            await kvdbApi.setEntry(kId, "key1", enc.encode("p"), enc.encode("p"), enc.encode("d"), 0);

            const expectError = async (fn: any) => { try { await fn(); } catch { return; } throw new Error("Expected error"); };
            await expectError(async () => await kvdbApi.getEntry(kId, "invalid"));

            // Force key gen on KVDB
            await kvdbApi.updateKvdb(kId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), 1, false, true);
    ...
    test('Listing entries keys with invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await expectError(async () => await kvdbApi.listEntriesKeys("invalid", {skip:0, limit:1, sortOrder:"desc"}));
            await expectError(async () => await kvdbApi.listEntriesKeys(contextId, {skip:0, limit:-1, sortOrder:"desc"})); // Using ctx ID as placeholder
            await expectError(async () => await kvdbApi.listEntriesKeys(contextId, {skip:0, limit:0, sortOrder:"desc"}));
            // @ts-ignore
            await expectError(async () => await kvdbApi.listEntriesKeys(contextId, {skip:0, limit:1, sortOrder:"invalid"}));
        }, args);
    });
    ...
    test('Listing entries keys with valid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const list1 = await kvdbApi.listEntriesKeys(kId, {skip: 4, limit: 1, sortOrder: "desc"});
            const list2 = await kvdbApi.listEntriesKeys(kId, {skip: 1, limit: 1, sortOrder: "desc"});
            const list3 = await kvdbApi.listEntriesKeys(kId, {skip: 0, limit: 3, sortOrder: "asc"});
    ...
    test('Listing entries with invalid input data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await expectError(async () => await kvdbApi.listEntries("invalid", {skip:0, limit:1, sortOrder:"desc"}));
            await expectError(async () => await kvdbApi.listEntries(contextId, {skip:0, limit:-1, sortOrder:"desc"}));
            await expectError(async () => await kvdbApi.listEntries(contextId, {skip:0, limit:0, sortOrder:"desc"}));
            // @ts-ignore
            await expectError(async () => await kvdbApi.listEntries(contextId, {skip:0, limit:1, sortOrder:"invalid"}));
        }, args);
    });
    ...
    test('Setting entry with valid/invalid input data', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const expectError = async (fn: any) => { try { await fn(); } catch { return; } throw new Error("Expected error"); };
            await expectError(async () => await kvdbApi.setEntry(kId, key, enc.encode("p"), enc.encode("p"), enc.encode("d2"), 0));
            ...
            const conn2 = await Endpoint.connect(users.u2.privKey, solutionId, bridgeUrl);
            const api2 = await Endpoint.createKvdbApi(conn2);
            await expectError(async () => await api2.setEntry(kId, key, enc.encode("p"), enc.encode("p"), enc.encode("fail"), 2));
    ...
    test('Trying to accessing unavailable data', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            await expectError(async () => await api2.updateKvdb(kId, [u1Obj], [u1Obj], enc.encode("p"), enc.encode("p"), 1, true, false));
            await expectError(async () => await api2.deleteKvdb(kId));
            await expectError(async () => await api2.getEntry(kId, "k1"));
            await expectError(async () => await api2.listEntries(kId, {skip:0, limit:1, sortOrder:"desc"}));
            await expectError(async () => await api2.setEntry(kId, "k1", enc.encode("p"), enc.encode("p"), enc.encode("d"), 1));
            await expectError(async () => await api2.deleteEntry(kId, "k1"));
    ...
    test.fixme('Trying to accessing data using publicAccount', async ({ page, backend, cli }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const conn1 = await Endpoint.connect(users.u1.privKey, solutionId, bridgeUrl);
            const api1 = await Endpoint.createKvdbApi(conn1);
            const kId = await api1.createKvdb(contextId, [{userId:users.u1.id, pubKey:users.u1.pubKey}], [{userId:users.u1.id, pubKey:users.u1.pubKey}], new TextEncoder().encode("p"), new TextEncoder().encode("p"));
            await api1.setEntry(kId, "k1", new TextEncoder().encode("p"), new TextEncoder().encode("p"), new TextEncoder().encode("d"), 0);
            ...
            await expectError(async () => await apiPub.listKvdbs(contextId, {skip:0, limit:1, sortOrder:"desc"}));
            await expectError(async () => await apiPub.createKvdb(contextId, [], [], new Uint8Array(0), new Uint8Array(0)));
    ...
    test('Creating kvdb with owner only policy', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            const kId = await api1.createKvdb(contextId, usersArr, usersArr, enc.encode("p"), enc.encode("p"), policy);
            ...
            let u2Access = false;
            try { await api2.getKvdb(kId); u2Access = true; } catch {}
    ...
    test('Updating kvdb with owner only policy', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, solutionId, contextId, users }) => {
            ...
            // Create
            const kId = await api1.createKvdb(contextId, [{userId:users.u1.id, pubKey:users.u1.pubKey}], [{userId:users.u1.id, pubKey:users.u1.pubKey}], enc.encode("p"), enc.encode("p"));
            ...
            // Update Policy
            await api1.updateKvdb(kId, usersArr, usersArr, enc.encode("p"), enc.encode("p"), 1, true, true, policy);
