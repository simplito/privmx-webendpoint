...
test.describe('CoreTest: Connection & Contexts', () => {
    ...
    test('Creating multiple instances of connection', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, user1, user2, solutionId }) => {
            ...
            // 4. List Contexts
            const listContexts_1 = await connection_1.listContexts({skip: 0, limit: 10, sortOrder: "desc"});
            const listContexts_2 = await connection_2.listContexts({skip: 0, limit: 10, sortOrder: "desc"});
    ...
    test('Disconnecting multiple instances of connection', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, user1, user2, solutionId }) => {
            ...
            const listContexts_AfterU2Disc = await connection_1.listContexts({skip: 0, limit: 1, sortOrder: "desc"});
            ...
            const listContexts_Reconnect = await connection_3.listContexts({skip: 0, limit: 1, sortOrder: "desc"});
    ...
    test.fixme('Connecting to public user', async ({ page, backend }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, solutionId }) => {
            ...
            await expectError(async () => {
                const res = await connection.listContexts({skip: 0, limit: 100, sortOrder: "asc"});
                console.log(res);
            });
    ...
    test('Listing context with invalid input data', async ({ page, backend }) => {
        ...
        await page.evaluate(async ({ bridgeUrl, user }) => {
            ...
            await expectError(async () => {
                // @ts-ignore
                await connection.listContexts({skip: 0, limit: 1, sortOrder: "desc", lastId: "bad_id"});
            });
    ...
    test('Listing context and active user status validation', async ({ page, backend, cli }) => {
        ...
        const result = await page.evaluate(async ({ bridgeUrl, user1, user2, solutionId, contextId }) => {
            const Endpoint = window.Endpoint;
            await Endpoint.setup("../../dist/assets");
            ...
            const usersInfo_1 = await conn1.listContextUsers(contextId, {skip: 0, limit: 10, sortOrder: "asc"});
            ...
            const usersInfo_2 = await conn2.listContextUsers(contextId, {skip: 0, limit: 10, sortOrder: "asc"});
        ...
        const u1_p1 = result.usersInfo_1.readItems.find((u: any) => u.user.userId === args.user1.id);
        const u2_p1 = result.usersInfo_1.readItems.find((u: any) => u.user.userId === args.user2.userId);
        ...
        const u1_p2 = result.usersInfo_2.readItems.find((u: any) => u.user.userId === args.user1.id);
        const u2_p2 = result.usersInfo_2.readItems.find((u: any) => u.user.userId === args.user2.userId);
