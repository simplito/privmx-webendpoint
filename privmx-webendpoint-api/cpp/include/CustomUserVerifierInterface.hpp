...
class CustomUserVerifierInterface
    : public virtual endpoint::core::UserVerifierInterface {
public:
  CustomUserVerifierInterface(int interfaceBindId)
      : endpoint::core::UserVerifierInterface(),
        _interfaceBindId(interfaceBindId) {}
  std::vector<bool> verify(
      const std::vector<endpoint::core::VerificationRequest> &request) override;

private:
  void printErrorInJS(const std::string &msg);
  Poco::Dynamic::Var callVerifierOnJS(const std::string &methodName,
