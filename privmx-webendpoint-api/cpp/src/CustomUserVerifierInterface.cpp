...
EM_JS(void, verifier_caller,
      (emscripten::EM_VAL name_handle, emscripten::EM_VAL val_handle,
       emscripten::EM_VAL val_bindId, int promise_id),
      {
        let name = Emval.toValue(name_handle);
        let params = Emval.toValue(val_handle);
        let bindId = Emval.toValue(val_bindId);

        window.userVerifierBinder[bindId]
            .userVierifier_verify(params)
            .then(response =
                      >
                      {
                        let ret = {status : 1, buff : response, error : ""};
                        Module.ccall('AsyncEngine_onSuccess', null,
                                     [ 'number', 'number' ],
                                     [ promise_id, Emval.toHandle(ret) ]);
                      })
            .catch(error = > {
              console.error("Error on userVerifier_verify call from C for",
                            params, error);
              let ret = {status : -1, buff : "", error : error.toString()};
              Module.ccall('AsyncEngine_onError', null, [ 'number', 'number' ],
                           [ promise_id, Emval.toHandle(ret) ]);
            });
      });
...
Poco::Dynamic::Var CustomUserVerifierInterface::callVerifierOnJS(
  ...
  auto ftr = AsyncEngine::getInstance()->callJsAsync(
      [&](int id) {
        ...
        verifier_caller(jsName.as_handle(), jsParams.as_handle(),
                        jsBindId.as_handle(), id);
      },
      ThreadTarget::Worker);
  return ftr.get();
}
...
std::vector<bool> CustomUserVerifierInterface::verify(
  ...
  Poco::Dynamic::Var resultVar =
      callVerifierOnJS("userVerifier_verify", serializedRequest);
