...
export class EventApiNative extends BaseNative {
  async newApi(connectionPtr: number): Promise<number> {
    return this.runAsync<number>((taskId) =>
      this.api.lib.EventApi_newEventApi(taskId, connectionPtr),
    );
  }
  ...
  async emitEvent(
    ptr: number,
    args: [string, UserWithPubKey[], string, Uint8Array],
  ): Promise<void> {
  ...
  async subscribeFor(ptr: number, args: [string[]]): Promise<string[]> {
    return this.runAsync<string[]>((taskId) =>
      this.api.lib.EventApi_subscribeFor(taskId, ptr, args),
    );
  }
  async unsubscribeFrom(ptr: number, args: [string[]]): Promise<void> {
    return this.runAsync<void>((taskId) =>
      this.api.lib.EventApi_unsubscribeFrom(taskId, ptr, args),
    );
  }
  async buildSubscriptionQuery(
    ptr: number,
    args: [string, EventsEventSelectorType, string],
  ): Promise<string> {
    return this.runAsync<string>((taskId) =>
      this.api.lib.EventApi_buildSubscriptionQuery(taskId, ptr, args),
    );
  }
}
