...
class Bindings {
    public:
    static emscripten::val callJSRawSync2(emscripten::val& name, emscripten::val& params);
    static void printErrorInJS2(std::string& msg);

    template <typename T>
    static T callJSSync(emscripten::val& name, emscripten::val& params) {
        emscripten::val result = callJSRawSync2(name, params);
        int status = result["status"].as<int>();
        if (status < 0) {
            auto errorString = result["error"].as<std::string>();
            printErrorInJS2(errorString);
            throw std::exception();
        }
