...
/**
 * @internal Raw BIP-39 result shape returned by the WASM layer — carries a raw ExtKey
...
/**
 * @internal Raw WASM wrapper for the C++ CryptoApi — holds and forwards raw pointers.
...
export class CryptoApiNative extends BaseNative {
    ...
    async fromMnemonic(ptr: number, args: [string, string]): Promise<BIP39> {
        const bipNative = await this.runAsync<BIP39Native>((taskId) =>
            this.api.lib.CryptoApi_fromMnemonic(taskId, ptr, args),
        );
        return this.convertBIP(bipNative);
    }
    ...
    async fromEntropy(ptr: number, args: [Uint8Array, string]): Promise<BIP39> {
        const bipNative = await this.runAsync<BIP39Native>((taskId) =>
            this.api.lib.CryptoApi_fromEntropy(taskId, ptr, args),
        );
        return this.convertBIP(bipNative);
    }
    ...
    async entropyToMnemonic(ptr: number, args: [Uint8Array]): Promise<string> {
        return this.runAsync<string>((taskId) =>
            this.api.lib.CryptoApi_entropyToMnemonic(taskId, ptr, args),
        );
    }

    async mnemonicToEntropy(ptr: number, args: [string]): Promise<Uint8Array> {
        return this.runAsync<Uint8Array>((taskId) =>
            this.api.lib.CryptoApi_mnemonicToEntropy(taskId, ptr, args),
        );
    }

    async mnemonicToSeed(ptr: number, args: [string, string]): Promise<Uint8Array> {
        return this.runAsync<Uint8Array>((taskId) =>
            this.api.lib.CryptoApi_mnemonicToSeed(taskId, ptr, args),
        );
    }
