...
export class ActiveSpeakerDetector {
  ...
  onFrame({ id, rms, timestamp }: FrameInput): SpeakerState[] {
    ...
    state.emaRms =
      this.opts.rmsEmaAlpha * rms +
      (1 - this.opts.rmsEmaAlpha) * state.emaRms;
    ...
    const adaptiveThreshold =
      state.noiseFloor + this.opts.thresholdOffset;
  ...
  private selectActiveSpeakers(now: number): SpeakerState[] {
    ...
    for (const [id, state] of this.speakers.entries()) {
      state.active = (now - state.lastAboveThresholdTs <= this.opts.activityWindowMs) && (now - state.activeSince < this.opts.holdMs);
    }
